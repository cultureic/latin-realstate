...
// } from "../libraries/CircuitAttributeHandlerV2.sol";
...
contract SelfHappyBirthday is SelfVerificationRoot, Ownable {
    ...
    event EuidBonusMultiplierUpdated(
        ...
        Ownable(_msgSender())
    {
        ...
        SelfUtils.UnformattedVerificationConfigV2
            memory _verificationConfig = SelfUtils
            ...
            IIdentityVerificationHubV2(identityVerificationHubAddress)
    ...
    function customVerificationHook(
        ...
        if (_isWithinBirthdayWindow(output.attestationId, output.dateOfBirth)) {
            ...
            if (output.attestationId == AttestationId.EU_ID_CARD) {
                finalAmount =
                    (claimableAmount * euidBonusMultiplier) /
                    BASIS_POINTS;
            }
    ...
    function _isWithinBirthdayWindow(
        ...
        string memory dobInThisYear = string(
            abi.encodePacked("25", month, day)
        );
        uint256 dobInThisYearTimestamp = Formatter.dateToUnixTimestamp(
            dobInThisYear
        );
